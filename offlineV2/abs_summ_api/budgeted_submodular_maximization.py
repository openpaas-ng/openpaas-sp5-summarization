...
def selection(compressions, utterances, stopwords, wv, config, summary_size):
    param = config['BSM']
    ...
    core_rank_scores_of_meeting = cr.get_core_rank_scores(
        lists_of_terms,
        window_size=param.getint('w'),
        overspanning=param.getboolean('overspanning'),
        weighted=param.getboolean('weighted')
    )
    ...
    kmeans_clusters_dict_of_meeting = kmeans_clusters_dict

    summary = [re.sub(' +', ' ', sentence).strip().lower().split(' ') for sentence in compressions]
    summary_of_meeting = summary
    ...
    for sentence in summary:
        summary_stemmed.append(
            utils.clean_text(
                ...
                pos_filtering=False, stemming=True,
                lower_case=True
            )
        )
    summary_stemmed_of_meeting = summary_stemmed

    cut = sentence_extraction_submodularity(
        summary_of_meeting,
        summary_stemmed_of_meeting,
        core_rank_scores_of_meeting.keys(),
        # round up to avoid carrying many decimals (to improve efficiency)
        np.round(np.array(core_rank_scores_of_meeting.values()) / sum(core_rank_scores_of_meeting.values()), 4),
        to_stem=False,
        budget=summary_size,
        scaling_factor=param.getfloat('scaling_factor'),
        weighted_sum_concepts=True,
        negative_terms=None,
        lamda=param.getfloat('lambda'),
        kmeans_diversity_score=True,
        kmeans_clusters_dict=kmeans_clusters_dict_of_meeting
    )
