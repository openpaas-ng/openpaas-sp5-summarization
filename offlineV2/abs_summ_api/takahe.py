...
class word_graph:
    ...
    def __init__(
        ...
        if self.language == 'en':
            self.wn_language = 'eng'
        elif self.language == 'fr':
            self.wn_language = 'fra'
        ...
        if self.tfidf_coverage:
            ...
            self.community_tf_dict = utils.term_frequencies(cleaned_document.split(' '))
    ...
    def synonym_nodes(self, node):
        ...
        if pos == '':
            return syn_nodes
        for syn in wn.synsets(word, pos=pos, lang=self.wn_language):
    ...
    def common_hypernym_nodes(self, node):
        ...
        for gnode in self.graph.nodes():
            ...
            if ref_pos == pos:
                for gnode_syn in wn.synsets(ref_word, pos=ref_pos, lang=self.wn_language):
    ...
    def best_candidate_similarity(self, candidates, node_to_add):
        ...
        pos = self.wn_tagging(tag)
        syn_to_add = wn.synsets(word, pos, lang=self.wn_language)[0]  # Get the most common sense synset
        ...
        for tmp_node, tmp_hyp in candidates:
            ...
            tmp_syn = wn.synsets(tmp_word, pos=tmp_pos, lang=self.wn_language)[0]  # Get the most common sense synset

            # score_of_common_hyp = similarity(common_hyp, new_word_to_add) * similarity(common_hyp, node_in_graph)
